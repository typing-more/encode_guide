...
更有趣和有用的mask是**edgemasks**和**debanding specific masks**。对于**edgemasks**，VapourSynth用户具有很大的优势，因为**kgf.retinex_edgemask**令人难以置信的准确，有着不可思议的结果。该**edgemasks**获取源图像，使用retinex算法提高暗区的对比度和亮度，然后将输出的TCanny蒙版层叠加在Kirsch蒙版上。两个常见用途是去色带和抗锯齿。
```python
