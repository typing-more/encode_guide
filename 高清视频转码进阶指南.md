...
这个例子中，我假设我们工作在16bit下。**std.Binarize**在此执行的内容是，使我们的位深所允许的每个小于5000的值变为最小值，而使每个大于5000的值成为最大值。这意味着将复制源上每个在5000亮度以上像素点，通常这被称为**luma mask**，即亮度遮罩。**luma mask**用于去色带，一般只有深色区域需要这些处理。
