...
- --ref 设置每个P帧可以作为参考的前一帧的数量。如果不关心硬件兼容性(和/或正在进行10位编码)，那么将其设置为16。虽然对性能的影响很大，但大多数时候还是值得的。计算你可以使用的数字，或者如果你正在使用所提供的CLI输入，在结束的时候它将通过x264为您计算。尽可能使用最大值。
  ...
  最大的ref值的计算方法如下: 对于 --level 4.1，根据H.264标准，最大的DPB(解码图像缓冲区）大小是12288 kb。由于每一帧都以YV12格式存储，即每像素存储1.5字节，所以1920x1088的帧就是1920×1088×1.5 = 3133440字节= 3060kb。12288÷3060kb = 4.01568627，所以可以使用最多4个参考帧。记住，计算时把两个维度都取整为mod16值，即使你没有编码mod16！
